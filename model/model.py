...
class Model:
    ...
    def load(self):
        ...
        if self.name == 'densenet161':
            self.net = DenseNet()
            state_dict = torch.load(root + '/data/densenet161.pt',
                map_location='cpu')
    ...
    def set_target(self, layer=None, filter=None, cl=None):
        if cl is not None and layer is not None and filter is None:
